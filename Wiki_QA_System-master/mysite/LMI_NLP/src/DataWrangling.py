...
class DataWrangler:
    ...
    def DataWranglerDf(self,df):
        ...
        # changed the diff char
        #removed_sp_char_Df['Wikipedia_Paragraphs'] = [re.sub(r"[^a-zA-Z0-9]+", ' ', doc) for doc in df['Wikipedia_Paragraphs']]
        removed_sp_char_Df['Wikipedia_Paragraphs'] = [re.sub(r" /[^a-zA-Z0-9 àâäèéêëîïôœùûüÿçÀÂÄÈÉÊËÎÏÔŒÙÛÜŸÇ]/", ' ', doc) for doc in df['Wikipedia_Paragraphs']]
